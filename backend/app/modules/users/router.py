...
async def list_users(
    ...
    if current_user.role not in [UserRole.LIBRARY_ADMIN.value, UserRole.LIBRARIAN.value]:
         raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.patch("/{user_id}/role", response_model=UserResponse, dependencies=[Depends(require_minimum_role(UserRole.LIBRARY_ADMIN))])
