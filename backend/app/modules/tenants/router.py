...
async def list_tenants(limit: int = 100, offset: int = 0, db: DBSession = None):
...
async def create_tenant(data: TenantCreate, db: DBSession = None):
...
async def get_tenant(tenant_id: UUID, db: DBSession = None):
...
async def update_tenant(tenant_id: UUID, data: TenantUpdate, db: DBSession = None):
...
async def delete_tenant(tenant_id: UUID, db: DBSession = None):
