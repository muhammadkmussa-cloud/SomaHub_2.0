...
class LoanService:
    ...
    async def issue_loan(
        self, tenant_id: UUID, issued_by_user_id: UUID, data: LoanCreate
    ) -> Loan:
        ...
        if copy.status != "available":
            raise HTTPException(
                ...
                detail="Book copy is not available",
            )
    ...
    async def return_loan(self, loan_id: UUID, tenant_id: UUID) -> Loan:
        ...
        if loan.status not in ["issued", "overdue"]:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail="Loan cannot be returned"
            )
        ...
        if copy:
            ...
            if book:
                await self.book_repo.update(
                    book,
                    available_copies=min(book.total_copies, book.available_copies + 1),
                )
    ...
    async def mark_lost(self, loan_id: UUID, tenant_id: UUID) -> Loan:
        ...
        if loan.status not in ["issued", "overdue"]:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Loan cannot be marked lost",
            )
        await self.fine_repo.create(
            tenant_id,
            borrower_id=loan.borrower_id,
